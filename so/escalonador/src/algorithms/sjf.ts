...
export function runSJF(inputs: ProcessInput[]): SimulationResult {
  if (inputs.length === 0) {
    return {
  ...
  while (finishedCount < totalProcesses) {
    // 1. SJF não-preemptivo: se não há processo em CPU, seleciona o pronto com menor duração total
    if (currentlyRunning === null) {
      ...
      if (ready.length > 0) {
        ...
        const tied = ready.filter((p) => p.duration === minDuration);
    ...
    if (currentlyRunning) {
      ...
      if (currentlyRunning.remainingTime === 0) {
        ...
        finishedCount += 1;
        currentlyRunning = null; // Não-preemptivo terminou, permite nova seleção
      }
    }
